...
public class EmailRecordServiceImpl implements EmailRecordService {
    ...
    public void SendMailForPreRegist(UserSession userSession, List <Integer> list,Integer templateId) throws Exception {
        ...
        for (Integer integer : list) {
            ...
            redisClusterService.setex ( ShortUrl.shortUrl ( baseShortUrl + "?" + preEmploymentVo.getEmploymentId () +"?"+ templateId ),
            ...
            String[] paramArr = {preEmploymentVo.getUserName (),preEmploymentVo.getApplicationPosition (),
                    ShortUrl.shortUrl ( baseShortUrl + "?" + preEmploymentVo.getEmploymentId () +"?"+ templateId) };
            //发送邮件
